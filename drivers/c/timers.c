...
bool gp_timer_wait(uint32_t base_addr, uint32_t ticks)
{
  ...
  //*********************
	...
	// Set the interval of Timer A to be the value of parameter 'ticks'
	gp_timer->TAILR = ticks;
	
	// Ckear a Timer A timeout
	gp_timer->ICR |= TIMER_ICR_TATOCINT;
	
	// Start only Timre A
	gp_timer->CTL |= TIMER_CTL_TAEN;
	
	// Busy-wait until a Timer A timeout occurs
...
bool gp_timer_config_32(uint32_t base_addr, uint32_t mode, bool count_up, bool enable_interrupts)
{
  ...
  //*********************
	...
	// Set timer to be 32-bits wide
	gp_timer->CFG = TIMER_CFG_32_BIT_TIMER;
		
	// Clear the mode bits (Don't modify any bits other then the mode bits)
	gp_timer->TAMR &= ~TIMER_TAMR_TAMR_M;
	
	// Set the timer's mode
	gp_timer->TAMR |= (mode & TIMER_TAMR_TAMR_M);
		
	// Set the direction of the general purpose register
	gp_timer->TAMR |= (count_up & TIMER_TAMR_TACDIR);
	
	// Add require if statement to enable or disable Timer A timeout interrupts
